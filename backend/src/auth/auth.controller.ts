...
export class AuthController {
  ...
  private setRefresh(res: Response, token: string) {
    res.cookie('refresh_token', token, { httpOnly: true, secure: this.config.get('NODE_ENV', { infer: true }) === 'production', sameSite: 'lax', path: '/api/v1/auth', maxAge: this.config.get('JWT_REFRESH_TTL_DAYS', { infer: true }) * 86_400_000 });
  }
  ...
  @Post('logout') async logout(@Req() req: Request, @Res({ passthrough: true }) res: Response) { const result = await this.auth.logout(req.cookies?.refresh_token); res.clearCookie('refresh_token', { path: '/api/v1/auth' }); return result; }
  ...
  @Get('google') google(@Res() res: Response) {
    ...
    res.cookie('oauth_state', state, { httpOnly: true, sameSite: 'lax', secure: this.config.get('NODE_ENV', { infer: true }) === 'production', maxAge: 600_000, path: '/api/v1/auth/google/callback' });
